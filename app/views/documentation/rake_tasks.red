...
1. First, CC.rb loads all *.rake files from [cruise]/tasks/, and then the Rakefile of your_project.
   Then it invokes <code>cc:build</code> task (defined in [cruise]/tasks/cc_build.rake). That task sets
   <em>RAILS_ENV</em> to <code>test</code> (unless your Rakefile already set it to something else), looks at other
   Rake tasks loaded, and decides what to do.
